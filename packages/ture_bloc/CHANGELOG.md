...
- **BREAKING**: remove `initialState` override in favor of providing the initial state via super ([#1304](https://github.com/mit-73/true_bloc/issues/1304)).
- **BREAKING**: Remove `BlocSupervisor` and rename `BlocDelegate` to `BlocObserver`.
- feat: support `null` states ([#1312](https://github.com/mit-73/true_bloc/issues/1312)).
- refactor: bloc to extend [cubit](https://pub.dev/packages/cubit) rather than `Stream`.
- feat: ignore newly added events after bloc is closed ([#1236](https://github.com/mit-73/true_bloc/issues/1236)).
...
- Ignore newly added events after bloc is closed ([#1236](https://github.com/mit-73/true_bloc/issues/1236)).
...
- Remove `rxdart` dependency ([#821](https://github.com/mit-73/true_bloc/pull/821))
- Replace `transformStates` with `transformTransitions` ([#840](https://github.com/mit-73/true_bloc/pull/840))
- Fix null `stackTrace` in `onError` ([#963](https://github.com/mit-73/true_bloc/pull/963))
...
- Allow blocs to finish processing pending events on `close` ([#639](https://github.com/mit-73/true_bloc/issues/639))
- Documentation Updates

# 1.0.1

- Bugfix: Exceptions thrown in `onTransition` are passed to `onError` and should not break bloc functionality ([#641](https://github.com/mit-73/true_bloc/issues/641))
...
- Bloc extends `Stream<State>` ([#558](https://github.com/mit-73/true_bloc/issues/558))
  - `bloc.state.listen` -> `bloc.listen`
  - `bloc.currentState` -> `bloc.state`
- Bloc implements `Sink<Event>` ([#558](https://github.com/mit-73/true_bloc/issues/558))
...
- Removed Bloc `event` Stream ([#326](https://github.com/mit-73/true_bloc/issues/326))
- Renamed `transform` to `transformEvents`
- Added `transformStates` ([#382](https://github.com/mit-73/true_bloc/issues/382))
...
- Deprecated Bloc `event` Stream ([#326](https://github.com/mit-73/true_bloc/issues/326))
- Documentation Updates
...
- `BlocSupervisor().delegate = ...` is now `BlocSupervisor.delegate = ...` ([#304](https://github.com/mit-73/true_bloc/issues/304)).

# 0.13.0

`Bloc` and `BlocDelegate` Improvements, new Features, and Documentation Updates.

- Improved `dispose` to ignore pending events ([#257](https://github.com/mit-73/true_bloc/issues/257)).
- Exposed `event` stream on `Bloc` similar to `state` stream to expose a `Stream` of `dispatched` events ([#259](https://github.com/mit-73/true_bloc/issues/259)).
- Update to use `rxdart` version `^0.22.0` ([#265](https://github.com/mit-73/true_bloc/issues/265)).
- `BlocDelegate` methods include a reference to the `Bloc` instance ([#259](https://github.com/mit-73/true_bloc/issues/259)).
- Added `onEvent` to `Bloc` and `BlocDelegate` ([#259](https://github.com/mit-73/true_bloc/issues/259)).
